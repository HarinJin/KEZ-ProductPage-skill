...
<img src="docs/assets/update-2026-07-22.png" width="500" alt="2026-07-22 업데이트 — 18개 에이전트 전수 감사 반영: 수치 SSOT·전체 조립 검증·인터뷰 강화·T15 복원·예제 placeholder 정비" />

<details>
<summary>텍스트로 보기 (2026-07-22)</summary>

- **SSOT** — 수치 규칙의 정본을 SKILL.md 하드 게이트 표 한 곳으로 통일. 문서별 체크리스트는 값을 재서술하지 않고 참조만 합니다(문서 간 드리프트 차단).
- **VERIFY** — 작업 흐름에 전체 조립 검증 단계(전체 스트립 스크린샷으로 밴드 리듬·액센트 반복·톤 연속성 확인)와 반려 시 재결정 루프(피드백 → 결정 슬롯 분류 → 재제작) 신설.
- **ASK** — 인터뷰 강화: 기획서에서 답이 나온 슬롯은 재질문하지 않는 훅, 이미지 자산 인벤토리(해상도·누끼 여부), 독립 질문은 한 메시지에 몰아 묻기, 착수 게이트에 페이지 아키타입 확정 추가.
- **FIX** — 함정 색인 T15(layoutAlign silent 리셋) 복원으로 죽은 참조 해소, 상호참조(§·T번호) 전수 정비, 인접 블록 템포 게이트를 기계 판정형으로 전환.
- **CODE** — 예제 코드의 크기 리터럴을 브랜드 placeholder(`BODY`/`DISPLAY`)로 교체 — 복붙해도 하드 게이트 위반이 생기지 않습니다.

</details>

> 업데이트 카드는 [`docs/update-card/update-card.html`](docs/update-card/update-card.html) 양식으로 만듭니다 — 텍스트만 바꿔 재렌더하면 됩니다(렌더 명령은 파일 상단 주석).
...
<img src="docs/assets/full-page.png" width="320" alt="데모 전체 — 히어로·구조·사용법 3섹션 세로 스택" />
...
완성 예시: [`references/brands/demo-browser-ui.md`](detail-page-core/references/brands/demo-browser-ui.md) — 위 [데모](#데모--이-스킬로-만든-페이지) 페이지가 이 프로파일로 제작되었습니다.
...
이 절차의 실제 결과물은 [데모](#데모--이-스킬로-만든-페이지) 섹션에서 볼 수 있습니다.
